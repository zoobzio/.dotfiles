...
1. Run `go build ./...` — if it doesn't build, stop
2. Run `go test -race ./...` — observe what passes, what fails, what's missing
3. Work through each domain — hunt for gaps, weak patterns, and false confidence
4. For each finding, ask: "If I introduced a bug here, would any test catch it?"
