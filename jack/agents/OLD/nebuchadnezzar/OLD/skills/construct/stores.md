...
Each variant has specific patterns and requirements:

- **Structured data with queries** (users, orders, documents) → see `stores-database.md`
- **Files or blobs** (images, uploads, serialized objects) → see `stores-bucket.md`
- **Simple key-value lookups** (cache, sessions, feature flags) → see `stores-kv.md`
- **Embeddings for similarity search** (semantic search, recommendations) → see `stores-index.md`
