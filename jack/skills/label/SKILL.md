...
- Reference agent names or character identities
- Reference crew roles or internal team structure
...
| Label | Description | Applied When |
|-------|-------------|-------------|
| `rejected` | Pull request was rejected | PR closed without merging |

Outcome labels are terminal — they indicate an issue that is not currently active. Issues with `rejected` are not selected for work by the loop. The user directs the team back to them explicitly.
